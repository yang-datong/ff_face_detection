    ...
    if ((dir = opendir(folderPath.c_str())) != NULL) {
        while ((ent = readdir(dir)) != NULL) {
            if (ent->d_type == DT_REG) {  // 判断是否是普通文件
                ...
                if(FindImages(subFolderPath, faceCascade, newSize, images, labels, labelsName, index) == -1){
...
int FaceRecognizer::FillData(vector<cv::Mat>& images, vector<int>& labels) {
    int index = 0;
    int status;
    status = FindImages(_folderPath,_faceCascade,_newSize,images,labels,labelsName,index);
    if(status == -1){
        LOGE("FindImages Failed");
        return -1;
    }
...
JNIEXPORT int JNICALL JNI_Initialization(JNIEnv *env, jobject thi, jstring workPath,jstring dataDirectory,jstring cascadeFile,jint useArithmetic) {
    ...
    gFace = new FaceRecognizer(gWorkPath,gDirectory,gNewSize,gCascadeFile,gArithmetic);
...
JNIEXPORT int JNICALL JNI_JustSaveFaceImage(JNIEnv *env, jobject thi,jstring oldFaceImagePath) {
...
static JNINativeMethod nativeMethods[] = {
    ...
    {"JNI_JustSaveFaceImage", "(Ljava/lang/String;)I", (void*)JNI_JustSaveFaceImage},
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    jclass clazz = env->FindClass("com/rl/ff_face_detection_yj/FaceRecognize");
    if (clazz == nullptr) {
        return -1;
    }
